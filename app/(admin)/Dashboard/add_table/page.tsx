...
type Action = { type: "add"; data: Table } | { type: "delete"; id: string };
...
export default function TablesPage() {
      ...
      if (response.success && response.data) {
        setTables(
          response.data.map((table) => ({
            ...table,
            availableDate: new Date(table.availableDate).toISOString(), // แปลงเป็น string
          }))
        );
      } else {
  ...
  const [optimisticTables, setOptimisticTables] = useOptimistic<Table[], Action>(
    tables,
    ...
    if (response.success && response.data) {
      startTransition(() => {
        setOptimisticTables({
          ...
          data: {
            ...response.data,
            availableDate: new Date(response.data.availableDate).toISOString(), // แปลงเป็น string
          },
        });
      });

      // รีเซ็ตค่าหลังเพิ่มสำเร็จ
      setTableNumber("");
      setSeatingCapacity("");
      setAvailableDate("");
    } else {
        ...
        <table className="w-full border-collapse border border-gray-200 rounded-lg shadow-md">
          ...
          <tbody>
            {optimisticTables.length > 0 ? (
              ...
              <tr>
                <td colSpan={4} className="p-4 text-center text-gray-500">ไม่มีโต๊ะในระบบ</td>
              </tr>
            )}
          </tbody>
        </table>
